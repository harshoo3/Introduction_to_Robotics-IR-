...
class Fabrik:
    def __init__(self, joint_posi, tolerance: float):
        if tolerance <= 0:
            raise ValueError("tolerance must be > 0")
        self.joints = joint_posi
        self.tolerance: float = tolerance
        self.length_links = []

        joint_a = joint_posi[0]
        for joint_b in joint_posi[1:]:
            self.length_links.append(np.linalg.norm(joint_a - joint_b))
            joint_a = joint_b

        if any([ll <= 0 for ll in self.length_links]):
            raise ValueError("link lengths must be > 0")

        self.lengths = self.length_links
        self.max_len = sum(self.length_links)
...
if __name__ == "__main__":
    ...
    iterations = fab.move_to(np.array(dest))
    print("The Results are as follows:")
    print("The Iterations-> ", iterations)
    print("The Angles->", fab.angles_deg())
    print("The Link position->", fab.joints)
    print("The Goal Position->", dest)